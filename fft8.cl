/* FFT
 * command: python -m fftsynth.generator --radix 2 --depth 3 --fpga --fma
 */
...
void fft_8_ps( float2 * restrict s0, float2 * restrict s1, float2 * restrict s0_in, float2 * restrict s1_in, float2 * restrict s0_out, float2 * restrict s1_out)
{
    int wp = 0;
    ...
    for ( uint2_t k = 0; k != 3; ++k )
    {
        ...
        for ( uint3_t i = 0; i != 4; ++i )
        {
            ...
            fft_2( s0, s1, s0_in, s1_in, s0_out, s1_out, k == 0, k == 2, MODR(i),  a + 0 * j, a + 1 * j, wp);
...
void fft_8()
{
    ...
    float2 s0[4];
    float2 s0_in[4], s0_out[4];
    float2 s1[4];
    float2 s1_in[4], s1_out[4];

    for ( uint4_t j = 0; j != 8; ++j )
    {
        int i = transpose_2(j);
        int p = parity_2(i);
