...
void print_subgrid_diff(
    idg::Array4D<std::complex<float>>& subgrids1,
    ...
    unsigned i)
{
    ...
    for (unsigned c = 0; c < std::min(nr_correlations, (unsigned) PRINT_MAX_NR_CORRELATIONS); c++) {
        std::cout << ">> correlation: " << c << std::endl;

        for (unsigned y = 0; y < std::min(height, (unsigned) PRINT_MAX_HEIGHT); y++) {
            for (unsigned x = 0; x < std::min(width, (unsigned) PRINT_MAX_WIDTH); x++) {
    ...
    unsigned i)
{
    ...
    for (unsigned time = 0; time < std::min(nr_timesteps, (unsigned) PRINT_MAX_NR_TIMESTEPS); time++) {
        std::cout << "time: " << time << std::endl;
        for (unsigned chan = 0; chan < std::min(nr_channels, (unsigned) PRINT_MAX_NR_CHANNELS); chan++) {
            idg::Visibility<std::complex<float>> visibility = visibilities(i, time, chan);
            std::cout << std::fixed;
            std::cout << std::setprecision(1);
            std::cout << "chan " << chan << ": ";
    ...
    unsigned i)
{
    ...
    for (unsigned time = 0; time < std::min(nr_timesteps, (unsigned) PRINT_MAX_NR_TIMESTEPS); time++) {
        std::cout << "time: " << time << std::endl;
        for (unsigned chan = 0; chan < std::min(nr_channels, (unsigned) PRINT_MAX_NR_CHANNELS); chan++) {
            idg::Visibility<std::complex<float>> visibility1 = visibilities1(i, time, chan);
            idg::Visibility<std::complex<float>> visibility2 = visibilities2(i, time, chan);
            std::cout << std::fixed;
            std::cout << std::setprecision(1);
            std::cout << "chan " << chan << ": ";
